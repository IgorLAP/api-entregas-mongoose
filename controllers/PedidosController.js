...
class PedidoController {
    ...
    static async editar(req, res) {
        ...
        if (id && id.length < 24) {
            res.status(400).json({
                message:
                    "Parâmetro ID não corresponde com o padrão de 24 caracteres",
            });
            return;
        }
        ...
        for (let i of id) {
            if (i > "f") {
                res.status(400).json({
                    message:
                        "Paramêtro ID não corresponde com o padrão hexadecimal",
                });
                return;
            }
        }
    ...
    static async verStatus(req, res) {
        ...
        if (!pedido) {
            res.status(406).json({
                message:
                    "Não encontramos um pedido associado à esse entregador",
            });
            return;
        }
    ...
    static async attStatus(req, res) {
        ...
        res.status(200).json({
            status: `Entrega de ${pedido.nomeEntregador} alterou o status para ${pedido.status}`,
        });
    }
}
